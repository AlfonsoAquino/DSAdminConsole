...
public class PlessoSearchModel {
	...
	public synchronized ArrayList<Statistica> getPlesso(String codicePlesso) {

		ArrayList<Statistica> stats=new ArrayList<Statistica>();
		Connection connection = null;
		PreparedStatement preparedStatement = null;
		String logQuery="SELECT * FROM statitisticadislessia where codicePlesso=?";
		int temp=0;
		try {
			connection = (Connection) ds.getConnection();
			preparedStatement = (PreparedStatement) connection.prepareStatement(logQuery);
			preparedStatement.setString(1, codicePlesso);
